...
ISR(TIMER2_COMPA_vect) {
  ...
  if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer)) {
    ...
    currentPitch = ypr_temp[1];
    currentRoll = ypr_temp[2];
    // 三筆平均濾波
...
void loop() {
  ...
  if (pid_computed) {
    ...
    setMotorSpeed(motor_cmd); // 如果不想馬達動，改成 setMotorSpeed(0);
    pid_computed = false;
  }

  // 3. 序列埠列印 (修改成您想要的格式)
  static unsigned long lastPrint = 0;
  if (millis() - lastPrint > 100) { // 改成 100ms 印一次，太快眼睛會看不清楚
    lastPrint = millis();
    
    // 轉換成角度 (原本是徑度)
    
    Serial.print("yaw = ");
    Serial.print(currentYaw * 57.296); 
    Serial.print("\t");
    
    Serial.print("pitch = ");
    Serial.print(currentPitch * 57.296);
    Serial.print("\t");
    
    Serial.print("roll = ");
    Serial.println(currentRoll * 57.296);
  }
}